...
# Clang is required for compiling sysdefs.c and running the binary which is not possible while cross compiling
TERMUX_PKG_DEPENDS="clang, libcrypt, libffi, openssl, readline"
...
termux_step_make_install() {
	...
	install ../bin/{balance,httpGate,psh,ssl,vip,watchdog} -t $TERMUX_PREFIX/lib/picolisp
...
termux_step_create_debscripts() {
	...
	clang $TERMUX_PREFIX/lib/picolisp/src/sysdefs.c -D_OS="\"Android\"" -D_CPU="\"$TERMUX_ARCH\"" -o $TERMUX_PREFIX/tmp/sysdefs-picolisp
	$TERMUX_PREFIX/tmp/sysdefs-picolisp > $TERMUX_PREFIX/lib/picolisp/sysdefs
	rm $TERMUX_PREFIX/tmp/sysdefs-picolisp
	EOF
